...
import { accounts, members, sessions, users, verifications, workspaces } from '../db/schema';
...
export function createProvider(db: AdapterDb, options: AuthOptions = {}) {
  return betterAuth({
    ...
    // Workspace = organization plugin (members, roles). Invitations ship with the
    // invite flow (requires sendInvitationEmail); teams/dynamic roles come later.

...
- **No test-only code in production modules**: no `*ForTesting` hooks, seams, or swappable singletons. Production exposes plain factories (`createProvider(db)` → `createAuthService(provider)` / `createWorkspaceService(provider)`, `createTrpcHandler(deps)`); a composition root binds them once (`auth/instance.ts`), and tests compose the same factories with pglite. If a test can't reach something, fix the design (inject the dependency), don't add a seam.

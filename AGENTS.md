...
- **Backend absolute imports** (lint-enforced): reach across directories via the `#backend/*` subpath (a Node `package.json` `imports` map, `#backend/* → ./src/*.ts`), never by climbing `../`. Same-directory `./` siblings stay relative; cross-package stays `@mocco/common/*`. `#` subpath imports are package-internal by design — they resolve natively under tsc/vitest/Next without touching the locked `exports` contract. `^\.\./` is banned (mirrors the frontend `@/`, #52).

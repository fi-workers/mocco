...
- **API surfaces are thin adapters over services**: the frontend↔backend surface is tRPC (`/api/trpc`); the vendor auth surface is better-auth's own fetch handler (`/api/auth/[...all]`). A transport (a tRPC router, or the `ext/` REST surface) carries no business logic — it parses at the boundary and delegates to the domain service, so no logic is duplicated across surfaces. Never expose tRPC to external/third-party consumers: external inbound (GitHub webhooks, any public API) gets its own REST surface — the GitHub App webhook (slice 3b) is live on it — and only a genuine public API is versioned (`/v1`); webhooks are not.
...
- **Repository per table for DB-owning domains** (lint-enforced; [ADR 0012](./docs/adr/0012-repository-per-table-for-db-owning-domains.md)): a domain that owns `mocco_` tables centralizes its drizzle queries in per-table repos (`domain/<d>/repos/<t>.repo.ts`, instance-injected). A service reaches the DB only through its repos — never `drizzle-orm`/`schema` directly (banned in `domain/**/*Service.ts`) — with `find*` (never throws) / `get*` (throws `EntityNotFoundError`) naming and workspace-scoped queries; the service maps `EntityNotFoundError` to its domain error. Shared `infra/db/rows.ts` helpers: `getOrThrow` (lookup) vs `expectOne` (single-row write). Vendor-mediated domains (auth) are exempt.
- **Env access is centralized** (lint-enforced in the backend): `packages/backend/src/infra/config/env.ts` is the only `process.env` reader — a lazy, zod-validated surface (`getEnv()`). Never read `process.env` inline; add new vars to the schema.
  - Env file layout (`packages/frontend/env/.env.local` vs `.env`), the `with-env` loader, `SERVICE_DOMAIN` (the canonical app host, replaces `AUTH_URL`), and the Tailscale tailnet-access recipe are documented in [docs/reference/env.md](./docs/reference/env.md).
...
- **Absolute imports, no relative paths** (lint-enforced, every package): every internal import uses that package's tsconfig-`paths` alias — `@backend/*` in `packages/backend`, `@frontend/*` in `packages/frontend` (both `→ ./src/*`) — never a relative `./` or `../` (the `^\.` regex is banned). Cross-package stays `@mocco/*` (the workspace packages' public `exports`). The frontend tsconfig also carries a resolution-only `@backend/* → ../backend/src/*` cross-map so the bundler can resolve the backend package's own internal imports when it inlines it; frontend source must **not** use `@backend/*` (lint-banned) — it reaches the backend only through the public `@mocco/backend/*` exports. vitest resolves the aliases natively (`resolve.tsconfigPaths`, no plugin).

...
- **Every PR is green and installable on a fresh clone**: format (`yarn format:check`), lint (`--max-warnings 0`), ts-check, tests (pglite, docker-free), frontend build, `yarn install` without lockfile drift — the same set CI enforces (`.github/workflows/ci.yml`).

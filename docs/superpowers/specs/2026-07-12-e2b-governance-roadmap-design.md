...
## Revision (2026-07-12) — supersedes parts of §5/§11 below

A design review sharpened the model. Where this note conflicts with the sections below, **this note wins** (the sections are kept for context):

- **Pipeline vs workflow.** A **pipeline** is Mocco's governance orchestration (steps + gates, declared in `.mocco.yml`); a **workflow** is the team's own executor unit (a GitHub Actions `.github/workflows/*.yml`, a shell script, …) that a step *references* (`with: { workflow: … }`) and the executor runs. Mocco owns pipelines, not workflows; its value is the gates between steps.
- **The `.mocco.yml`'s real home is the repo, read at a run's commit.** The config that governs a deploy is the file **as it exists at that run's commit** — fetched + parsed **at run time** (per run = per deploying commit, NOT every commit). Not authored/stored in Mocco.
- **Each run is self-contained.** A run **embeds its own config snapshot** (`raw_yaml` + parsed `definition` + `content_hash` + `commit_sha`) so it is independent, immutable, and its own complete audit record. **Drop the shared/deduped `pipeline_versions` table** — the KB-sized config duplicated per run beats coupling runs to a shared row. (`content_hash` still used for integrity + "which runs share a config".)
- **Slice 1 is preview-only.** With no run/commit yet, there is nothing to snapshot, so slice 1 = **parse + preview a pasted `.mocco.yml`** (show steps or issues), **no persistence**. Real storage begins at slice 2's self-contained runs. (Implemented on branch `feat/pipeline-preview`: `pipeline.preview` tRPC + a preview page, reusing the `@mocco/common/mocco-config` zod schema + `MoccoConfigParser` from the original slice-1 work.)

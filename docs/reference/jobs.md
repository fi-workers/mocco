...
code_refs:
  ...
  - packages/backend/src/domain/jobs/instance.ts
...
Export the domain's handlers from a pure factory in `domain/<x>/jobs.ts`
(`createNotificationHandlers(deps): JobHandler[]`) and spread it into the registry in
`runtime/jobs.ts` (`createJobRunner`). The factory takes repos/services as arguments and never
...
A job can run more than once, and two runs can overlap:

- a run that crashed or timed out is retried;
- a run still going past `ctx.deadline` (`locked_until`) is reclaimed by the next tick and started
  again **while the first run is still running**. The first run's result is then discarded
  (`lost`), but its side effects already happened.

So every side effect must be idempotent (keyed writes, upserts, a provider idempotency key), and a
long handler should check `ctx.now() < ctx.deadline` before starting another side effect.

## Enqueueing

Domains depend on the `JobQueue` port (`domain/jobs/ports.ts`). Their composition root injects
`getJobQueue()` from `domain/jobs/instance.ts`:
...
| `kick` | Also run the job right away in `waitUntil`. The table is the fallback if that run dies. Deduped enqueues are not kicked. Not allowed with `executor`. |
| `executor` | The caller's transaction (`tx`). The job commits or rolls back with the caller's writes. |

### Enqueueing inside a transaction

**Never enqueue inside a transaction without passing it as `executor`.** Without it the insert asks
the pool for a second connection. Production's pool has exactly one (`max: 1` in
`infra/db/client.ts`), so the insert waits for the transaction, which waits for the insert: a
deadlock. With `executor`, the job is part of the transaction. Kick it only after the commit:

```ts
const { job } = await db.transaction(async tx => {
  await deliveries.insert(tx, delivery);
  return await queue.enqueue(deliverNotification, { deliveryId }, { executor: tx });
});
queue.kick(job.id);
```

`enqueue` throws if `kick: true` is combined with `executor`, because a kick inside the
transaction could run before the commit, or run a job that was rolled back.
...
- **The runner dies** → `reclaimExpired` (every tick) re-queues it once `locked_until` passes; the
  crashed attempt stays counted, the RetryAt streak resets, and a job with no attempts left goes
  `dead`.
...
Each phase is isolated: if one throws (say, a bad schedule row breaks `advanceDue`), it is logged
and listed in the report's `errors` as `{ phase, message }`, and the later phases still run, so
queued jobs keep draining.

### Timing invariant

`JobTiming` in `domain/jobs/policy.ts` keeps three limits ordered, and a unit test checks the
order:

`visibilityMs` (10 min) > `functionMaxDurationMs` (300 s) > `maxTickBudgetMs` (240 s) ≥
`defaultTickBudgetMs` (50 s).

- The lock must outlive the function holding it. Otherwise a run that is still going is reclaimed
  and started a second time.
- The tick must stop claiming before the platform kills the function, so the job it is on can
  finish. `JOBS_TICK_BUDGET_MS` is clamped to `maxTickBudgetMs`.
- `functionMaxDurationMs` mirrors `export const maxDuration = 300` in
  `packages/frontend/src/app/api/ext/[[...route]]/route.ts`. Next reads that value statically, so
  it must be a literal. A test fails if the two differ.
...
| `200` | The tick ran. Body: `{ reclaimed, scheduled, ran, outcomes: { succeeded, retried, deferred, dead, lost }, errors: [{ phase, message }] }`. |
...
| `JOBS_TICK_BUDGET_MS` | `50000` | How long a tick keeps claiming jobs. Clamped to `JobTiming.maxTickBudgetMs` (240 s). |
...
Two roots, so that no domain's `instance.ts` imports another's through the job registry:

- **`domain/jobs/instance.ts` → `getJobQueue()`** depends only on the DB, `JobRepo` and
  `PostgresJobQueue`. Any domain's `instance.ts` can inject it. For `kick`, it loads the runner
  with a dynamic `import('@backend/runtime/jobs')` at call time.

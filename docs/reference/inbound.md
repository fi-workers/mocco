...
| Step | Result |
|---|---|
| 0. Route: the key must match `^[A-Za-z0-9_-]{43}$`, the body must be at most 1 MB | Malformed key → `404 not found` before the body is read or the DB is touched; larger body → `413` |
...
| 4a. Hard ceiling: receipts of any outcome of the workspace in the last 24 h | At `INBOUND_HARD_LIMIT` (20,000, four times the daily limit) or more → `429 too many deliveries`, nothing written |
...
| 6. Quota (soft): published receipts of the workspace in the last 24 h | At `INBOUND_DAILY_LIMIT` (5,000) or more → the receipt becomes `over_quota` → `202` |
| 7. Publish the event with `dedupeKey = inbound:<receiptId>`, mark the receipt `published` with `domain_event_id` | `202 accepted` |

A delivery that gets past the ceiling also updates the source's `last_received_at`, at most once a
minute per source; a failure of that update is logged and never fails the delivery.
`202` means **recorded**: delivery to Discord never runs on the request path. If the publish in
step 7 fails, the receipt stays `pending` (with `publish_attempts` counted) and the request is
still `202`. Anything unexpected (the DB, a secret that no longer opens) is a bare
`500 Internal server error`.

### Quota and hard ceiling

- **The daily quota is soft.** It is a count followed by a write, not a lock, so deliveries that
  arrive at the same moment can each pass the count and overshoot the limit by a few. It protects
  against runaway volume; it is not a billing meter.
- **The hard ceiling bounds the table.** Past `INBOUND_HARD_LIMIT` receipts of any outcome
  (ignored and over-quota included) in 24 hours, a delivery is refused with `429` and nothing is
  written, so a flood or a vendor stuck in a retry loop cannot grow `mocco_inbound_receipts`
  without limit. The refusal is logged at most once per workspace per 24-hour window (per
  function instance). The trace shows no receipt for a refused delivery.

### Logging

Log lines carry ids and error classes only: `{ receiptId, sourceId, error: <class>, code? }`,
where `code` is the driver's error code when there is one. Never the error object or its message,
because a failed query's message carries its parameters (the ingest key, the payload, a sealed
value). A secret that fails to open is logged with the source id.
...
| Kind | Schedule | Does |
|---|---|---|
| `inbound.republish-stale` | every 60 s (platform schedule) | Publishes `pending` receipts older than 60 s, up to 1,000 per run, fewest failed publishes first, then oldest, with the same dedupe key, and marks them `published`. The quota is not checked again. Each failure adds to `publish_attempts`; after 5 the receipt is marked `ignored` (`publishing failed 5 times`), and a payload the catalog rejects (or that no longer parses) is marked `ignored` at once, so a receipt that can never publish never holds up newer ones. |
...
Reads need workspace membership (`WorkspaceService.assertMember`); writes need an owner or admin
(`WorkspaceService.assertAdmin`, one lookup of the caller's own role that also proves membership;
`FORBIDDEN` for a plain member). A non-member gets `NOT_FOUND` on every procedure, and a source of
another workspace is `NOT_FOUND`.
...
- `mocco_inbound_receipts`: `seq` (bigserial), `workspace_id` (cascade), `source_id` (composite FK
  ...
  `publish_attempts`, `received_at`. `UNIQUE (source_id, external_id)`; indexes for the trace (`workspace_id, seq
  DESC`, `source_id, seq DESC`), the quota and ceiling (`workspace_id, received_at`), the
  republish scan (`publish_attempts, received_at` where pending) and the prune (`received_at`).

...
- **Shell & workspace routing.** `AppShell` is a slim Vercel-style top bar (logo + workspace switcher + **project switcher** + user menu) that **fetches its own** session + workspaces. Inside a workspace, `WorkspaceLayout` adds the left nav around the section content. The nav comes from the **product registry** (`lib/products.ts`): each section names the product it belongs to (or none), and sections of products the workspace hasn't enabled (`product.list`) are hidden. A new product screen is one registry entry plus its page. Workspace-scoped routes: `/workspaces/[id]` (dashboard) and `/workspaces/[id]/{projects,members,access,notifications,audit,products,settings}`. Project-scoped products live under `/workspaces/[id]/p/[projectId]/…` inside `ProjectLayout` (the project header and tabs from the registry's project sections); the project is in the path, so a reload lands on the same screen. Visiting one makes it active server-side (`setActive`, which also validates membership). `/workspaces` is a router, not a list — it jumps into a workspace when one exists (`?create` or zero shows the focused create view).
...
- **External inbound is on the App Router.** `app/api/ext/[[...route]]/route.ts` (the only `app/` file) delegates GET/POST to the backend's fetch-standard `extHandler` (a Hono app) — [ADR 0011](../adr/0011-external-api-surface-architecture.md). App and Pages routers coexist in Next 16; the "Pages Router" convention governs the **frontend UI**, not backend route handlers. Raw-body webhook verification (slice 3b) needs the App Router's untouched `Request`.
